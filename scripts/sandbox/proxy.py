...
def relay(source, destination):
    while True:
        try:
            ...
            # rustls-backed clients (uv >= 0.6, Node 22 with BoringSSL) do not
            # send a TLS close_notify before closing the connection.  Python's
            # ssl module raises SSLEOFError in that case; treat it as a normal
            # EOF so the relay exits cleanly instead of crashing the thread and
            # flooding the log with spurious "proxy request failed" lines.
            return
        if not chunk:
            return
...
def forward_https(conn, host, port, request):
    ...
    # Some TLS implementations (rustls as used by uv >= 0.6 and BoringSSL in
    # Node 22) do not send close_notify on shutdown.  Without this flag,
    # Python's ssl layer raises SSLEOFError on the recv() that follows --
    # relay() catches that too, but setting the flag avoids the exception path
    # entirely for the upstream socket.
